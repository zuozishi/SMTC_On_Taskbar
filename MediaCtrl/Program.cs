...
namespace CortanaMusicUpdate
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if(process.Length>=2)
            {
                MessageBox.Show("CortanaMusicUpdate is running!");
                return;//防止进程重复
            }
            timer.Interval = 2000;
        ...
        private static void Timer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                var FB2K = Process.GetProcessesByName("foobar2000");
                
                if (cloudmusic.Length == 0&& qqmusic.Length==0&& FB2K.Length == 0)
                {
                ...
                else if (FB2K.Length > 0)
                {
                    if (GetCortanaText() != GetFB2KText())
                    {
                        int oldx, oldy;
                        SetText(GetFB2KText());
                        oldx = Control.MousePosition.X;
        ...
        //获取曲名-FB2K
        static string GetFB2KText()
        {
            var musicbox = FindWindow("{97E27FAA-C0B3-4b8e-A693-ED7881E99FC1}", null);
            int musiclen = GetWindowTextLength(musicbox);
            StringBuilder windowName = new StringBuilder(musiclen + 1);
            GetWindowText(musicbox, windowName, windowName.Capacity);
            return windowName.ToString();
